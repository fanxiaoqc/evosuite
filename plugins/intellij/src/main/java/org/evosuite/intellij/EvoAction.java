 ...
 */
public class EvoAction<main> extends AnAction {

    public EvoAction() {
        //Modify by me
        //super("Run EvoSuite",
        super("NUT Hammer",
                "Open GUI dialog to configure and start running NUT Hammer to generate JUnit tests automatically",
                loadIcon());
    }

    private static Icon loadIcon(){
        try {
            Image image = ImageIO.read( EvoAction.class.getClassLoader().getResourceAsStream( "nuthammer.png" ));
    ...
    public void actionPerformed(AnActionEvent event) {
        ...
        String title = "NUT Hammer Plugin";
        Project project = event.getData(DataKeys.PROJECT);

        ToolWindowManager toolWindowManager = ToolWindowManager.getInstance(project);
        ToolWindow toolWindow = toolWindowManager.getToolWindow("NUT Hammer");

        final AsyncGUINotifier notifier = IntelliJNotifier.getNotifier(project);

        if (EvoSuiteExecutor.getInstance().isAlreadyRunning()) {
            Messages.showMessageDialog(project, "An instance of NUT Hammer is already running",
    ...
    private Map<String, Set<String>> getCUTsToTest(AnActionEvent event){
        ...
        for(VirtualFile virtualFile : event.getData(DataKeys.VIRTUAL_FILE_ARRAY)){
